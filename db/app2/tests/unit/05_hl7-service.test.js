...
describe('HL7 Service', () => {
  ...
  describe('CDA Document Creation', () => {
    ...
    const mockObservations = [
      { ENCOUNTER_NUM: 1, CONCEPT_CD: 'LOINC:8302-2', VALTYPE_CD: 'N', NVAL_NUM: 180, START_DATE: '2024-01-15' }
    ]
  ...
  describe('Document Text Generation', () => {
    ...
    it('should calculate date range correctly', async () => {
      ...
      const observations = [
        { ENCOUNTER_NUM: 1, CONCEPT_CD: 'TEST', START_DATE: '2024-01-16' }
      ]
  ...
  describe('Digital Signature and Hash', () => {
    it('should generate document hash', () => {
      ...
      const hash = hl7Service.generateDocumentHash(document)
    ...
    it('should generate consistent hash for same document', () => {
      const document = { test: 'data', id: '123' }
      const hash1 = hl7Service.generateDocumentHash(document)
      const hash2 = hl7Service.generateDocumentHash(document)
    ...
    it('should verify CDA document signature', () => {
      const document = { test: 'data' }
      const hash = hl7Service.generateDocumentHash(document)
      ...
      const isValid = hl7Service.verifyCda(hl7Document)
      expect(isValid).toBe(true)
    })

    it('should reject invalid CDA document', () => {
      const document = { test: 'data' }
      const hash = hl7Service.generateDocumentHash(document)
    ...
    it('should reject document without hash', () => {
      const hl7Document = {
        cda: { test: 'data' }
      }
  ...
  describe('HL7 Export', () => {
    ...
    it('should validate required input data', async () => {
      ...
      await expect(hl7Service.exportToHl7(invalidOptions))
    ...
    it('should handle export errors gracefully', async () => {
      ...
      await expect(hl7Service.exportToHl7(mockExportOptions))
        .rejects.toThrow('HL7 export failed: Test error')
    })
  })
  ...
  describe('Data Extraction', () => {
    ...
    it('should extract observation with JSON blob value', () => {
      ...
      // Write test output
      writeTestOutput('hl7-observation-extraction.json', {
        input: entry,
        output: observation
  ...
  describe('Error Handling', () => {
    ...
    it('should handle document signing errors', async () => {
      ...
      await expect(hl7Service.signDocument(document, privateKey, 'public'))
        .rejects.toThrow('Document signing failed')
    })

    it('should handle CDA verification errors', () => {
      const invalidDocument = null
      
      const result = hl7Service.verifyCda(invalidDocument)
