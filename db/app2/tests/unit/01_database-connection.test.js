...
describe('SQLiteConnection', () => {
  ...
  describe('Query Operations', () => {
    ...
    it('should execute SELECT queries with parameters', async () => {
      const result = await connection.executeQuery('SELECT * FROM users WHERE id = ?', [123])
    ...
    it('should reject queries when not connected', async () => {
      ...
      await expect(connection.executeQuery('SELECT 1', [])).rejects.toThrow('Database not connected')
    })
  })
  ...
  describe('Command Operations', () => {
    ...
    it('should execute INSERT commands', async () => {
      const result = await connection.executeCommand('INSERT INTO users (name, email) VALUES (?, ?)', ['John Doe', 'john@example.com'])
    ...
    it('should execute UPDATE commands', async () => {
      const result = await connection.executeCommand('UPDATE users SET name = ? WHERE id = ?', ['Jane Doe', 1])
    ...
    it('should execute DELETE commands', async () => {
      const result = await connection.executeCommand('DELETE FROM users WHERE id = ?', [1])
    ...
    it('should reject commands when not connected', async () => {
      ...
      await expect(connection.executeCommand('INSERT INTO test VALUES (?)', ['value'])).rejects.toThrow('Database not connected')
    })
  })
  ...
  describe('Transaction Operations', () => {
    ...
    it('should reject transactions when not connected', async () => {
      ...
      const commands = [{ sql: 'INSERT INTO test VALUES (?)', params: ['value'] }]

      await expect(connection.executeTransaction(commands)).rejects.toThrow('Database not connected')
    })
  })
