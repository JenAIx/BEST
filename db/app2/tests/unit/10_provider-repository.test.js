...
describe('ProviderRepository', () => {
  ...
  describe('createProvider', () => {
    ...
    it('should throw error if PROVIDER_ID is missing', async () => {
      ...
      await expect(providerRepository.createProvider(providerData)).rejects.toThrow('PROVIDER_ID is required for provider creation')
    })
    ...
    it('should throw error if NAME_CHAR is missing', async () => {
      ...
      await expect(providerRepository.createProvider(providerData)).rejects.toThrow('NAME_CHAR is required for provider creation')
    })
    ...
    it('should use custom SOURCESYSTEM_CD and UPLOAD_ID when provided', async () => {
      ...
      expect(mockConnection.executeCommand).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO PROVIDER_DIMENSION'), expect.arrayContaining(['CUSTOM', 999, 'PROV001', 'Dr. John Smith']))
    })
  })
  ...
  describe('findByProviderId', () => {
    it('should find provider by ID', async () => {
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith('SELECT * FROM PROVIDER_DIMENSION WHERE PROVIDER_ID = ?', ['PROV001'])
  ...
  describe('findByName', () => {
    it('should find providers by name pattern', async () => {
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith('SELECT * FROM PROVIDER_DIMENSION WHERE NAME_CHAR LIKE ? ORDER BY NAME_CHAR', ['%Smith%'])
  ...
  describe('findByPath', () => {
    it('should find providers by path pattern', async () => {
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith('SELECT * FROM PROVIDER_DIMENSION WHERE PROVIDER_PATH LIKE ? ORDER BY PROVIDER_PATH', ['%NEURO%'])
      expect(result).toEqual(mockProviders)
    })
  })
  ...
  describe('findByDepartment', () => {
    it('should find providers by department', async () => {
      const mockProviders = [{ PROVIDER_ID: 'PROV001', PROVIDER_PATH: 'UKJ/NEURO/ATTENDING' }]
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith('SELECT * FROM PROVIDER_DIMENSION WHERE PROVIDER_PATH LIKE ? ORDER BY NAME_CHAR', ['%NEURO%'])
      expect(result).toEqual(mockProviders)
    })
  })
  ...
  describe('findByOrganization', () => {
    it('should find providers by organization', async () => {
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith('SELECT * FROM PROVIDER_DIMENSION WHERE PROVIDER_PATH LIKE ? ORDER BY PROVIDER_PATH, NAME_CHAR', ['UKJ%'])
      expect(result).toEqual(mockProviders)
    })
  })
  ...
  describe('getProvidersWithObservationCounts', () => {
    it('should get providers with their observation counts', async () => {
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith(expect.stringContaining('LEFT JOIN OBSERVATION_FACT'))
  ...
  describe('getProvidersPaginated', () => {
    ...
    it('should apply search criteria', async () => {
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith(expect.stringContaining('NAME_CHAR LIKE ?'), expect.arrayContaining(['%Smith%', '%NEURO%', 'UKJ%', '%CARDIO%']))
    })
  ...
  describe('searchProviders', () => {
    it('should search providers by multiple fields', async () => {
      const mockProviders = [{ PROVIDER_ID: 'PROV001', NAME_CHAR: 'Dr. Smith', PROVIDER_PATH: 'UKJ/NEURO' }]
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith(expect.stringContaining('PROVIDER_ID LIKE ?'), ['%Smith%', '%Smith%', '%Smith%'])
      expect(result).toEqual(mockProviders)
    })
  ...
  describe('updateProvider', () => {
    ...
    it('should throw error if provider does not exist', async () => {
      ...
      await expect(providerRepository.updateProvider('NONEXISTENT', {})).rejects.toThrow('Provider with PROVIDER_ID NONEXISTENT not found')
    })
  })
  ...
  describe('findByRole', () => {
    it('should find providers by role', async () => {
      const mockProviders = [{ PROVIDER_ID: 'PROV001', NAME_CHAR: 'Dr. Smith', CONCEPT_BLOB: 'wissenschaftlicher Mitarbeiter' }]
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith('SELECT * FROM PROVIDER_DIMENSION WHERE CONCEPT_BLOB LIKE ? ORDER BY NAME_CHAR', ['%wissenschaftlicher Mitarbeiter%'])
      expect(result).toEqual(mockProviders)
    })
  })
  ...
  describe('getActiveProviders', () => {
    it('should get providers with recent activity', async () => {
      const mockProviders = [{ PROVIDER_ID: 'PROV001', NAME_CHAR: 'Dr. Smith' }]
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith(expect.stringContaining("o.START_DATE >= date('now', '-30 days')"))
      expect(result).toEqual(mockProviders)
    })
    ...
    it('should use default threshold of 30 days', async () => {
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith(expect.stringContaining("'-30 days'"))
    })
  })
  ...
  describe('getProviderPerformance', () => {
    it('should return provider performance metrics', async () => {
      ...
      expect(mockConnection.executeQuery).toHaveBeenCalledWith(expect.stringContaining('COUNT(o.OBSERVATION_ID) as totalObservations'), ['PROV001'])
