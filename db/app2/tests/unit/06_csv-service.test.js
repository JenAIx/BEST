...
describe('CSV Service', () => {
  ...
  describe('CSV Export', () => {
    ...
    it('should validate required input data', async () => {
      ...
      await expect(csvService.exportToCsv(invalidOptions)).rejects.toThrow('Patients data is required and must be an array')
    })
    ...
    it('should handle export errors gracefully', async () => {
      ...
      await expect(csvService.exportToCsv(exportOptions)).rejects.toThrow('CSV export failed: Test error')
    })
  })

  describe('CSV Structure Building', () => {
    const mockPatients = [{ PATIENT_NUM: 1, PATIENT_CD: 'TEST001' }]

    const mockVisits = [{ ENCOUNTER_NUM: 1, PATIENT_NUM: 1, START_DATE: '2024-01-15' }]
