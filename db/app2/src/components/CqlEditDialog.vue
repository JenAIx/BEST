            ...
            <q-card-section class="q-pa-md" style="max-height: calc(90vh - 120px); overflow-y: auto">
                ...
                <div class="row q-gutter-md">
                    <div class="col-12">
                        <div class="row items-center q-mb-md">
                            ...
                            <q-space />
                            <q-btn v-if="hasChanges || !formData.JSON_CHAR" round icon="refresh" color="primary"
                                @click="compileRule" :loading="compiling">
                        ...
                        <!-- Mode Toggle -->
                        <div class="q-mb-md">
                            <q-btn-toggle v-model="editorMode" :options="editorModeOptions" color="primary"
                                toggle-color="primary" />
                        </div>

                        <!-- CQL Source Editor -->
                        <div v-show="editorMode === 'CQL_CHAR'" class="q-mb-md">
                            <q-input v-model="formData.CQL_CHAR" type="textarea" outlined label="CQL Source Code"
                                rows="15" class="cql-editor" @update:model-value="onCqlChange" />
                        </div>

                        <!-- JSON/ELM Viewer -->
                        <div v-show="editorMode === 'JSON_CHAR'" class="q-mb-md">
                            <q-input v-model="formData.JSON_CHAR" type="textarea" outlined
                                label="Compiled JSON/ELM (Read-only)" rows="15" readonly class="json-viewer" />
                        </div>
                    </div>
                </div>
                ...
                <div class="row q-gutter-md">
                    <div class="col-12">
                        ...
                        <div class="row items-center q-mb-md">
                            <q-btn color="secondary" icon="play_arrow" label="Run Test" @click="runTest"
                                :loading="testing" :disable="!canTest" />
                        </div>
                            ...
                            <template v-slot:body-cell-value="props">
                                <q-td :props="props">
                                    <q-input v-model="props.row.value" dense borderless
                                        :type="props.row.type === 'number' ? 'number' : props.row.type === 'date' ? 'date' : 'text'" />
                                </q-td>
                            </template>

                            <template v-slot:body-cell-type="props">
                                <q-td :props="props">
                                    <q-select v-model="props.row.type" :options="typeOptions" dense borderless
                            ...
                            <template v-slot:body-cell-result="props">
                                <q-td :props="props">
                                    <div v-if="props.row.result" class="text-center">
                                        <q-icon :name="props.row.result.check ? 'check_circle' : 'error'"
                                            :color="props.row.result.check ? 'positive' : 'negative'" size="sm" />
...
import { unstringify_json, stringify_json, unstringify_char, stringify_char } from 'src/utils/sql-tools.js'
