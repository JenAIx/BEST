<template>
  <div v-if="questionnaire" class="preview-survey">
    ...
    <div class="survey-header q-mb-lg">
      ...
      <div v-if="questionnaire.short_title" class="text-subtitle1 text-grey-7 q-mb-sm">Short Title: {{ questionnaire.short_title }}</div>
      ...
      <div class="survey-info">
        <q-chip v-if="questionnaire.items?.length" dense color="primary" text-color="white" icon="quiz" class="q-mr-sm"> {{ questionnaire.items.length }} Questions </q-chip>
        <q-chip v-if="questionnaire.results?.method" dense color="secondary" text-color="white" icon="calculate" class="q-mr-sm"> Method: {{ questionnaire.results.method }} </q-chip>
        <q-chip v-if="questionnaire.results?.max_score" dense color="positive" text-color="white" icon="star"> Max Score: {{ questionnaire.results.max_score }} </q-chip>
      </div>
    </div>
    ...
    <div class="survey-questions">
      ...
      <q-list bordered separator class="rounded-borders">
        <q-item v-for="(item, index) in questionnaire.items" :key="`preview-item-${item.id || index}`" class="preview-question-item">
          <q-item-section>
            <!-- Question Number and Label -->
            <q-item-label class="question-header">
              <div class="row items-start no-wrap">
                <div class="col-auto">
                  <q-badge color="grey-6" :label="index + 1" class="q-mr-sm question-number" />
                </div>
                <div class="col">
            ...
            <q-item-label class="question-preview q-mt-sm q-ml-lg">
              <div class="preview-input-container">
                ...
                <div v-if="item.type === 'number'" class="preview-input">
                  <q-input outlined dense readonly :placeholder="getPreviewPlaceholder(item)" class="preview-field" />
                ...
                <div v-else-if="item.type === 'radio'" class="preview-input">
                  <div class="preview-radio-options">
                    <div v-for="(option, optIndex) in getRadioOptions(item)" :key="optIndex" class="preview-radio-option">
                      <q-radio :model-value="null" :val="option.value" :label="option.label" disable color="primary" />
                    </div>
                  </div>
                ...
                <div v-else-if="item.type === 'checkbox'" class="preview-input">
                  <div class="preview-checkbox-options">
                    <div v-for="(option, optIndex) in getCheckboxOptions(item)" :key="optIndex" class="preview-checkbox-option">
                      <q-checkbox :model-value="false" :label="option.label" disable color="primary" />
                    </div>
                  </div>
                ...
                <div v-else-if="item.type === 'slider'" class="preview-input">
                  <q-slider :model-value="getSliderDefault(item)" :min="item.min || 0" :max="item.max || 100" :step="item.step || 1" disable color="primary" class="q-mt-md" />
                ...
                <div v-else-if="item.type === 'date'" class="preview-input">
                  <q-input outlined dense readonly placeholder="Select date..." class="preview-field">
                ...
                <div v-else-if="item.type === 'time'" class="preview-input">
                  <q-input outlined dense readonly placeholder="Select time..." class="preview-field">
                    <template v-slot:append>
                ...
                <div v-else-if="item.type === 'multiple-radio'" class="preview-input">
                  <div class="preview-multiple-radio">
                    <div v-for="(group, groupIndex) in item.groups" :key="groupIndex" class="radio-group q-mb-md">
                      <div class="text-subtitle2 q-mb-sm">{{ group.label }}</div>
                      <div class="row">
                        <div v-for="(option, optIndex) in group.options" :key="optIndex" class="col-auto q-mr-md">
                          <q-radio :model-value="null" :val="option.value" :label="option.label" disable color="primary" />
                        </div>
                      </div>
                    </div>
        ...
        <q-card-section>
          ...
          <div v-if="questionnaire.results.ranges" class="q-mt-md">
            ...
            <div class="score-ranges">
              <q-chip v-for="(range, index) in questionnaire.results.ranges" :key="index" dense outline :color="getScoreRangeColor(index)" class="q-mr-sm q-mb-sm">
