    ...
    <div class="observation-field">
        <div class="field-header">
            <div class="field-label">
                ...
                <q-icon v-else-if="concept.system === 'SNOMED'" name="medical_services" size="14px" color="green"
                    class="q-ml-xs">
                    <q-tooltip>SNOMED Code</q-tooltip>
                </q-icon>
            </div>
            <div class="field-actions">
                <q-btn v-if="previousValue" flat round icon="content_copy" size="sm" color="grey-6"
        ...
        <div class="field-input">
            ...
            <div v-if="concept.valueType === 'N'" class="numeric-input">
                <q-input v-model.number="currentValue" type="number" :label="`Enter ${concept.name.toLowerCase()}`"
                    outlined dense :suffix="concept.unit" @update:model-value="onValueChange" :loading="saving">
            ...
            <div v-else-if="concept.valueType === 'T'" class="text-input">
                <q-input v-model="currentValue" :label="`Enter ${concept.name.toLowerCase()}`" outlined dense
                    @update:model-value="onValueChange" :loading="saving">
                    <template v-slot:prepend>
            ...
            <div v-else-if="concept.valueType === 'D'" class="date-input">
                <q-input v-model="currentValue" type="date" :label="`Enter ${concept.name.toLowerCase()}`" outlined
                    dense @update:model-value="onValueChange" :loading="saving">
                    <template v-slot:prepend>
                        <q-icon name="event" />
                    </template>
                </q-input>
            ...
            <div v-else-if="concept.valueType === 'S'" class="selection-input">
                <q-select v-model="currentValue" :options="getSelectionOptions()"
                    :label="`Select ${concept.name.toLowerCase()}`" outlined dense emit-value map-options
                    @update:model-value="onValueChange" :loading="saving">
                    <template v-slot:prepend>
                        <q-icon name="list" />
                    </template>
...
const onValueChange = async (newValue) => {
    ...
    try {
        ...
        currentValue.value = props.existingObservation?.value || ''
    } finally {
        saving.value = false
    }
}
...
const getSelectionOptions = () => {
    ...
    return commonOptions[props.concept.code] || [
...
watch(() => props.existingObservation, (newObs) => {
    if (newObs) {
        currentValue.value = newObs.value
