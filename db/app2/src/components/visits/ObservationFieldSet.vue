    ...
    <div class="field-set-section">
        <div class="field-set-header">
            ...
            <div class="field-set-actions">
                <q-btn flat icon="content_copy" label="Clone from Previous" size="sm" @click="showCloneDialog = true"
                    :disable="previousVisits.length === 0">
                    <q-tooltip>Copy values from a previous visit</q-tooltip>
                </q-btn>
                <q-btn flat icon="expand_more" size="sm" @click="collapsed = !collapsed"
        ...
        <q-slide-transition>
            <div v-show="!collapsed" class="field-set-content">
                <div class="observation-grid">
                    <ObservationField v-for="concept in fieldSetConcepts" :key="concept.code" :concept="concept"
                        :visit="visit" :patient="patient" :existing-observation="getExistingObservation(concept.code)"
                        :previous-value="getPreviousValue(concept.code)" @observation-updated="onObservationUpdated"
                        @clone-requested="onCloneRequested" />
                </div>

                <!-- Add custom observation -->
                <div class="add-custom-observation">
                    <q-btn flat icon="add" label="Add Custom Observation" @click="showAddCustomDialog = true"
                        class="full-width" style="border: 2px dashed #ccc" />
                </div>
            </div>
        </q-slide-transition>
        ...
        <q-dialog v-model="showCloneDialog">
            <q-card style="min-width: 400px">
                ...
                <q-card-section class="q-pt-none">
                    <q-select v-model="selectedPreviousVisit" :options="previousVisitOptions" option-label="label"
                        option-value="value" label="Select Previous Visit" outlined />

                    <div v-if="selectedPreviousVisit" class="q-mt-md">
                        <div class="text-subtitle2 q-mb-sm">Available observations:</div>
                        <q-list bordered>
                            <q-item v-for="obs in previousVisitObservations" :key="obs.conceptCode" clickable
                                @click="cloneObservation(obs)">
                                <q-item-section>
            ...
            <q-card style="min-width: 400px">
                ...
                <q-card-section class="q-pt-none">
                    <q-form class="q-gutter-md">
                        <q-input v-model="customObservation.name" label="Observation Name" outlined
                            :rules="[val => !!val || 'Name is required']" />
                        <q-select v-model="customObservation.valueType" :options="valueTypeOptions" label="Value Type"
                            outlined emit-value map-options />
                        <q-input v-model="customObservation.value"
                            :label="customObservation.valueType === 'N' ? 'Numeric Value' : 'Text Value'"
                            :type="customObservation.valueType === 'N' ? 'number' : 'text'" outlined />
                ...
                <q-card-actions align="right">
                    ...
                    <q-btn color="primary" label="Add" @click="saveCustomObservation"
                        :disable="!customObservation.name || !customObservation.value" />
                </q-card-actions>
            </q-card>
        </q-dialog>
    </div>
...
const fieldSetConcepts = computed(() => {
    ...
    return props.fieldSet.concepts?.map(conceptCode => {
...
const getConceptValueType = (conceptCode) => {
    ...
    const numericConcepts = [
        'LOINC:8480-6', 'LOINC:8462-4', 'LOINC:8867-4',
        'LOINC:8310-5', 'LOINC:9279-1', 'LOINC:2708-6'
    ]
...
const getExistingObservation = (conceptCode) => {
    return props.existingObservations.find(obs =>
        obs.conceptCode.includes(conceptCode.split(':')[1])
    )
}
