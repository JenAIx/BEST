<template>
    <AppInputDialog v-model="dialogVisible" title="Create New Concept" ok-label="Create Concept" cancel-label="Cancel"
        :loading="isSaving" :disable-ok="!isFormValid" @ok="onSubmit" @cancel="onCancel" size="lg">
        <q-form @submit.prevent="onSubmit" class="q-gutter-md">
            <!-- Source System Field (moved to top) -->
            <q-select v-model="formData.sourceSystem" label="Source System" outlined dense
                :options="sourceSystemOptions" :rules="[
                    val => !!val || 'Source system is required'
                ]" @update:model-value="onSourceSystemChange" hint="Source system (LOINC, ICD10, SNOMED-CT, etc.)" />

            <!-- Concept Path Field -->
            <q-input v-model="formData.conceptPath" label="Concept Path" outlined dense :rules="[
            ...
            ]" @update:model-value="onFieldChange" hint="Hierarchical path (e.g., \\LOINC\\CHEM\\Bld\\2947-0)">
            ...
            <div class="row q-gutter-sm">
                ...
                <div class="col">
                    <q-input v-model="formData.conceptCode" label="Concept Code" outlined dense :rules="[
                        val => !!val || 'Concept code is required',
                        val => val.length >= 3 || 'Concept code must be at least 3 characters'
                    ]" @update:model-value="onFieldChange" hint="Unique identifier">
                        <template v-slot:append>
                            <q-icon v-if="formData.sourceSystem === 'SNOMED-CT'" name="search"
                                @click="handleConceptCodeSearch" class="cursor-pointer">
            ...
            <!-- Name Field -->
            <q-input v-model="formData.name" label="Concept Name" outlined dense :rules="[
                val => !!val || 'Concept name is required'
            ]" @update:model-value="onFieldChange" hint="Human-readable name for the concept">
                <template v-slot:append>
                    <q-icon v-if="formData.sourceSystem === 'SNOMED-CT'" name="search" @click="handleConceptNameSearch"
            ...
            <!-- Value Type Field -->
            <q-select v-model="formData.valueType" label="Value Type" outlined dense :options="valueTypeOptions" :rules="[
                val => !!val || 'Value type is required'
            ]" @update:model-value="onValueTypeChange" hint="Type of value this concept represents" />
            ...
            <div v-if="formData.valueType === 'S'" class="q-pa-md bg-grey-1 rounded-borders">
                ...
                <div v-if="selectionAnswers">
                    {{ selectionAnswers }}
                    <q-icon v-if="selectionAnswers.includes('link:')" name="close" class="cursor-pointer q-ml-sm"
                        @click="unlinkAnswer()" />
            ...
            <!-- Unit Code Field -->
            <q-input v-model="formData.unitCode" label="Unit Code" outlined dense @update:model-value="onFieldChange"
                hint="Unit of measurement (optional, e.g., mg/dL, mmHg)" />

            <!-- Related Concept Field -->
            <q-input v-model="formData.relatedConcept" label="Related Concept" outlined dense
                @update:model-value="onFieldChange" hint="Related concept code (optional)" />

            <!-- Description Field -->
            <q-input v-model="formData.description" label="Description" outlined dense type="textarea" rows="3"
                @update:model-value="onFieldChange" hint="Detailed description of the concept" />
    ...
    <!-- Path Picker Dialog -->
    <PathPickerDialog v-if="showPathPicker" v-model="showPathPicker" :current-path="formData.conceptPath"
        @save="updateConceptPath" />
    ...
    <q-dialog v-model="showSNOMEDSearchDialog" persistent>
        <q-card style="min-width: 600px; max-width: 800px">
            ...
            <q-card-section>
                <!-- Search Input -->
                <q-input v-model="snomedSearchQuery" label="Search Term" outlined dense
                    placeholder="Enter search term (e.g., 'heart attack', 'diabetes')"
                    @keyup.enter="searchSNOMEDConcepts" class="q-mb-md">
                    <template v-slot:append>
                        <q-btn icon="search" color="primary" @click="searchSNOMEDConcepts"
                            :loading="snomedSearchLoading" />
                    </template>
                </q-input>
                ...
                <div v-if="snomedSearchResults.length > 0" class="q-mt-md">
                    <div class="text-subtitle2 q-mb-sm">
                        Found {{ snomedSearchResults.length }} concepts
                    </div>
                    <q-list class="bordered" style="max-height: 300px; overflow-y: auto">
                        <q-item v-for="result in snomedSearchResults" :key="result.conceptId" clickable v-ripple
                            ...
                            <q-item-section>
                                ...
                                <q-item-label v-if="result.fsn !== result.term" caption class="text-grey-5">
                                    FSN: {{ result.fsn }}
                                </q-item-label>
                            </q-item-section>
                            <q-item-section side>
                                <q-btn flat round dense icon="add" color="primary"
                                    @click.stop="selectSNOMEDConcept(result)">
                            ...
                            <q-tooltip class="bg-white text-black shadow-4" style="font-size: 12px; max-width: 400px">
                                <div class="q-pa-sm">
                                    ...
                                    <div v-if="result.description"><strong>Description:</strong> {{ result.description
                                        }}
                                    </div>
                                    <div><strong>Status:</strong> {{ result.active ? 'Active' : 'Inactive' }}</div>
                                    <div v-if="result.definitionStatus"><strong>Definition:</strong> {{
                                        result.definitionStatus }}</div>
                ...
                <div v-if="!snomedSearchLoading && snomedSearchQuery && snomedSearchResults.length === 0"
                    class="text-center text-grey-6 q-pa-md">
                    No concepts found for "{{ snomedSearchQuery }}"
                </div>
            </q-card-section>
...
const isFormValid = computed(() => {
    return formData.value.conceptCode.trim() !== '' &&
...
const getSNOMEDParent = (res) => {
    ...
    for (let relationship of relationships) {
        if (relationship.type && relationship.type.conceptId === '116680003' &&
            relationship.active && relationship.destinationId) {
...
const determineSNOMEDValueType = (conceptData) => {
    ...
    if (term.includes('disorder') || term.includes('disease') || term.includes('syndrome') ||
        term.includes('condition') || term.includes('abnormal')) {
        return 'F' // Finding
    }

    if (term.includes('observable') || term.includes('measurement') || term.includes('value') ||
        term.includes('level') || term.includes('count') || term.includes('concentration')) {
        return 'N' // Numeric
    }

    if (term.includes('substance') || term.includes('product') || term.includes('agent') ||
        term.includes('material')) {
        return 'T' // Text
    }
    ...
    if (id >= 404684003 && id <= 609096000) { // Clinical finding range
        return 'F' // Finding
    }

    if (id >= 71388002 && id <= 432102000) { // Procedure range
        return 'F' // Finding/Procedure
    }

    if (id >= 105590001 && id <= 766939001) { // Substance range
        return 'T' // Text
    }
