    ...
    <q-dialog v-model="dialogVisible" persistent>
        <q-card style="min-width: 600px; max-width: 800px">
            ...
            <q-card-section>
                ...
                <div class="q-mb-md">
                    <q-input v-model="selectedPath" label="Selected Path" outlined dense readonly
                        hint="Click on path segments below to navigate" />
                </div>

                <!-- Path Navigation -->
                <div class="q-mb-md">
                    <q-breadcrumbs separator="\" class="text-grey-8">
                        <q-breadcrumbs-el v-for="(segment, index) in pathSegments" :key="index"
                            :label="segment || 'Root'"
                ...
                <!-- Search Field -->
                <q-input v-model="searchQuery" outlined dense placeholder="Search path segments..." debounce="300"
                    @update:model-value="loadPathOptions" class="q-mb-md">
                ...
                <q-list class="bordered" style="max-height: 300px; overflow-y: auto">
                    <q-item-label header v-if="!loading && pathOptions.length > 0">
                        Available Path Segments
                    </q-item-label>

                    <q-item v-for="option in pathOptions" :key="option.path" clickable v-ripple
                        @click="selectPath(option.path)">
                ...
                <q-expansion-item icon="edit" label="Manual Path Entry" class="q-mt-md">
                    <q-card>
                        <q-card-section>
                            <q-input v-model="manualPath" label="Enter Path Manually" outlined dense
                                placeholder="\LOINC\CHEM\Bld\" hint="Path must start and end with backslash (\)" :rules="[
                                    val => !!val || 'Path is required',
                                    val => val.startsWith('\\') || 'Path must start with \\',
                                    val => val.endsWith('\\') || 'Path must end with \\'
                                ]" />
                            <div class="q-mt-sm">
                                <q-btn flat label="Use Manual Path" color="primary" :disable="!isValidManualPath"
                                    @click="useManualPath" />
                            </div>
                        </q-card-section>
                    </q-card>
...
const isValidManualPath = computed(() => {
    return manualPath.value &&
        manualPath.value.startsWith('\\') &&
        manualPath.value.endsWith('\\')
})
