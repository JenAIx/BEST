<template>
  <q-avatar :color="getValueTypeColor(valueType)" :text-color="getValueTypeTextColor()" :size="size" :class="avatarClass">
    <q-icon :name="getValueTypeIcon(valueType)" :size="iconSize" />
    <q-tooltip v-if="showTooltip" class="bg-grey-9"> {{ getValueTypeLabel(valueType) }} - {{ getValueTypeDescription(valueType) }} </q-tooltip>
  </q-avatar>
</template>
