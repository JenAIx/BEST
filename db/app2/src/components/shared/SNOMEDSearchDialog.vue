<template>
  <q-dialog v-model="dialogVisible" persistent>
    <q-card style="min-width: 600px; max-width: 800px">
      ...
      <q-card-section>
        <div class="text-caption text-grey-6 q-mb-md">Search the international SNOMED CT terminology for clinical concepts</div>

        <q-input v-model="localSearchQuery" outlined dense label="Search SNOMED CT concepts" placeholder="e.g., diabetes, hypertension, myocardial infarction..." class="q-mb-md" @keyup.enter="search">
          <template v-slot:prepend>
            <q-icon name="search" />
          </template>
          <template v-slot:append>
            <q-btn flat dense icon="search" @click="search" :loading="searching" color="primary" />
          </template>
        </q-input>
        ...
        <q-list v-else-if="searchResults.length > 0" bordered separator class="snomed-results">
          <q-item v-for="result in searchResults" :key="result.code" clickable @click="selectConcept(result)" class="snomed-result-item">
            <q-item-section>
              ...
              <q-item-label class="text-weight-medium text-primary">
                ...
                <!-- Hover info icon -->
                <q-icon name="info_outline" size="16px" class="q-ml-xs text-grey-5 hover-info-icon">
              ...
              <q-item-label caption class="text-blue-grey-7">
                ...
                <q-chip v-if="result.active" size="xs" color="positive" text-color="white" label="Active" class="q-ml-sm" />
              </q-item-label>

              <!-- Fully Specified Name (if different from preferred term) -->
              <q-item-label v-if="result.fsn && result.fsn !== result.display" caption lines="2" class="text-grey-7 q-mt-xs"> FSN: {{ result.fsn }} </q-item-label>
            </q-item-section>
      ...
      <q-card-actions align="right" class="q-pa-md">
        ...
        <div class="text-caption text-grey-6 q-mr-md">Powered by Snowstorm SNOMED CT API</div>
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
