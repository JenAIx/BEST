  ...
  <q-dialog v-model="dialogVisible" persistent>
    <q-card style="min-width: 700px; max-width: 900px">
      ...
      <q-card-section>
            ...
            <q-card-section>
              <q-form @submit.prevent="addAnswer" class="q-gutter-md">
                <div class="row q-gutter-md">
                  <div class="col">
                    <q-input v-model="newAnswer.code" label="Answer Code" outlined dense :rules="[(val) => !!val || 'Answer code is required']" hint="e.g., LA6113-0" />
                  </div>
                  <div class="col">
                    <q-input v-model="newAnswer.name" label="Answer Name" outlined dense :rules="[(val) => !!val || 'Answer name is required']" hint="e.g., Positive" />
                  </div>
                </div>

                <q-input v-model="newAnswer.description" label="Description (optional)" outlined dense type="textarea" rows="2" />

                <div class="row justify-end q-gutter-sm">
                  <q-btn flat label="Cancel" @click="cancelAddAnswer" />
                  <q-btn type="submit" label="Add Answer" color="primary" :disable="!newAnswer.code || !newAnswer.name" />
                </div>
              </q-form>
            </q-card-section>
          </q-card>
        ...
        <!-- Existing Answers -->
        <div class="text-subtitle2 q-mb-sm">Existing Answers ({{ answers.length }})</div>

        <q-table :rows="answers" :columns="columns" row-key="CONCEPT_CD" :loading="loading" :pagination="{ rowsPerPage: 10 }" flat bordered>
          <!-- Answer Code Column -->
        ...
        <div v-if="!loading && answers.length === 0" class="text-center text-grey-6 q-pa-lg">No answers found. Add some answers above.</div>
      </q-card-section>
      ...
      <q-card style="min-width: 400px">
        ...
        <q-card-section>
          <q-form @submit.prevent="saveEditAnswer" class="q-gutter-md">
            ...
            <q-input v-model="editingAnswer.NAME_CHAR" label="Answer Name" outlined dense :rules="[(val) => !!val || 'Answer name is required']" />

            <q-input v-model="editingAnswer.CONCEPT_BLOB" label="Description" outlined dense type="textarea" rows="3" />
          </q-form>
        </q-card-section>

        <q-card-actions align="right">
          <q-btn flat label="Cancel" @click="showEditDialog = false" />
          <q-btn flat label="Save" color="primary" @click="saveEditAnswer" :disable="!editingAnswer.NAME_CHAR" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-dialog>
