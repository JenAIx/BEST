<template>
    <AppInputDialog v-model="dialogVisible" title="Reset Password" ok-label="Save Password" cancel-label="Cancel"
        :loading="isSaving" :disabled="!isFormValid" @ok="onSubmit" @cancel="onCancel">
        <q-form @submit.prevent="onSubmit" class="q-gutter-md">
            <!-- New Password Field -->
            <q-input v-model="formData.newPassword" label="New Password" type="password" outlined dense :rules="[
...
const isFormValid = computed(() => {
    return formData.value.newPassword.length >= 5 &&
