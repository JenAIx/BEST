        ...
        <q-card style="min-width: 600px; max-width: 800px;">
            ...
            <q-card-section>
                <div class="text-body2 text-grey-6 q-mb-md">
                    Add observation to visit {{ visit?.encounterNum }} for {{ patient?.PATIENT_CD || 'this patient' }}
                </div>
                ...
                <div v-if="!selectedConcept" class="concept-search-section">
                    ...
                    <!-- Search Input -->
                    <q-input v-model="conceptSearchQuery" outlined dense placeholder="Search for medical concepts..."
                        clearable class="q-mb-md" :loading="searchLoading">
                    ...
                    <div v-if="recentConcepts.length > 0" class="q-mb-md">
                        ...
                        <div class="recent-concepts">
                            <q-chip v-for="concept in recentConcepts" :key="concept.CONCEPT_CD" clickable
                                @click="selectConcept(concept)" color="blue-1" text-color="primary" size="md"
                                class="q-ma-xs">
                                {{ concept.NAME_CHAR }}
                            </q-chip>
                        </div>
                    ...
                    <div v-if="searchResults.length > 0" class="search-results">
                        <div class="text-subtitle2 text-grey-7 q-mb-sm">
                            ...
                            <span v-if="totalResultsCount > displayedResults.length">
                                of {{ totalResultsCount }} (showing top {{ displayedResults.length }})
                            </span>
                            <span v-else-if="totalResultsCount > 0">
                                of {{ totalResultsCount }}
                            </span>
                        </div>
                        <q-list bordered class="rounded-borders" style="max-height: 300px; overflow-y: auto;">
                            <q-item v-for="concept in displayedResults" :key="concept.CONCEPT_CD" clickable
                                @click="selectConcept(concept)" class="concept-item">
                                <q-item-section>
                    ...
                    <!-- No Results -->
                    <div v-if="conceptSearchQuery && !searchLoading && searchResults.length === 0"
                ...
                <div v-else class="observation-form-section">
                        ...
                        <q-card flat bordered class="bg-blue-1">
                            <q-card-section class="q-pa-sm">
                                <div class="row items-center">
                                    <div class="col">
                                        <div class="text-weight-medium text-primary">{{ selectedConcept.NAME_CHAR }}
                                        </div>
                    ...
                    <q-form @submit="handleSubmit" class="q-gutter-md">
                        ...
                        <div class="row q-gutter-md">
                            <div class="col">
                                <q-input v-model="formData.START_DATE" type="date" label="Observation Date *" outlined
                                    dense :rules="[val => !!val || 'Date is required']">
                                    <template v-slot:prepend>
                                        <q-icon name="event" />
                                    </template>
                                </q-input>
                        ...
                        <div class="value-input-section">
                            ...
                            <div v-if="valueInputType === 'numeric'" class="row q-gutter-md">
                                <div class="col">
                                    <q-input v-model.number="formData.NVAL_NUM" type="number" label="Numeric Value"
                                        outlined dense step="any">
                                        <template v-slot:prepend>
                                            <q-icon name="numbers" />
                                        </template>
                                    </q-input>
                            ...
                            <div v-else-if="valueInputType === 'selection'">
                                <q-select v-model="formData.TVAL_CHAR" :options="getSelectionOptions()"
                                    label="Select Value" outlined dense emit-value map-options
                                    :loading="loadingSelectionOptions">
                                    <template v-slot:prepend>
                                        <q-icon name="list" />
                                    </template>
                                    <template v-slot:hint v-if="selectionOptions.length > 0">
                                        {{ selectionOptions.length }} option(s) available from concept hierarchy
                                    </template>
                                </q-select>
                            </div>
                            ...
                            <div v-else-if="valueInputType === 'file'">
                                <FileUploadInput v-model="fileData" :max-size-m-b="10"
                                    accepted-types=".txt,.png,.gif,.jpg,.jpeg,.pdf" @file-selected="handleFileSelected"
                        ...
                        <q-input v-model="formData.CATEGORY_CHAR" label="Category" outlined dense readonly>
                            ...
                            <template v-slot:hint>
                                Automatically set from selected concept
                            </template>
                        </q-input>
                        ...
                        <div v-if="valueInputType !== 'file'" class="row q-gutter-md">
                            <div class="col-4">
                                <q-input v-model="formData.QUANTITY_NUM" type="number" label="Quantity" outlined dense
                                    step="any">
                                    <template v-slot:prepend>
                            ...
                            <div class="col-4">
                                <q-input v-model.number="formData.CONFIDENCE_NUM" type="number" label="Confidence %"
                                    outlined dense min="0" max="100">
                                    <template v-slot:prepend>
                            ...
                            <div class="col-3">
                                <q-select v-model="formData.VALUEFLAG_CD" :options="valueFlagOptions" label="Value Flag"
                        ...
                        <!-- Notes -->
                        <q-input v-model="formData.OBSERVATION_BLOB" type="textarea" label="Notes" outlined dense
                            rows="3">
                            <template v-slot:prepend>
                                <q-icon name="notes" />
                            </template>
                        ...
                        <div class="row q-gutter-md q-mt-md">
                            ...
                            <q-btn type="submit" label="Create Observation" color="primary" :loading="loading"
                                :disable="!isFormValid" />
                        </div>
                    </q-form>
                </div>
...
const valueInputType = computed(() => {
    ...
    switch (valtype) {
        ...
        case 'R': return 'file' // Raw data type should use file upload
        case 'T':
        case 'A':
        default:
            return 'text'
