<template>
  <q-card class="observation-card" :class="{ 'editing-card': isEditing }" :data-value-type="observation.VALTYPE_CD || 'T'" flat bordered>
    <q-card-section class="q-pa-sm">
        ...
        <div class="q-gutter-sm">
          ...
          <div class="value-input-section">
            ...
            <div v-if="valueInputType === 'numeric'">
              <q-input v-model.number="editForm.nvalNum" label="Numeric Value" type="number" outlined dense class="q-mb-sm" step="any">
            ...
            <div v-else-if="valueInputType === 'date'">
              <q-input v-model="editForm.tvalChar" label="Date Value" type="date" outlined dense class="q-mb-sm">
                <template v-slot:prepend>
                  <q-icon name="event" />
                </template>
              </q-input>
            ...
            <div v-else-if="valueInputType === 'selection'">
              <q-select v-model="editForm.tvalChar" :options="getSelectionOptions()" label="Select Value" outlined dense emit-value map-options :loading="loadingSelectionOptions" class="q-mb-sm">
                <template v-slot:prepend>
                  <q-icon name="list" />
                </template>
                <template v-slot:hint v-if="selectionOptions.length > 0"> {{ selectionOptions.length }} option(s) available </template>
              </q-select>
            </div>
      ...
      <div v-else>
          ...
          <div class="col">
            ...
            <div class="observation-value q-mt-xs">
              ...
              <div v-if="observation.VALTYPE_CD === 'R' && observation.TVAL_CHAR" class="file-display">
                <div class="file-value-display text-primary clickable-value" @click="openFilePreview">
                  {{ getTvalDisplay(observation) }}
                  <q-tooltip class="bg-grey-9 text-white">
                    <span v-if="getTvalDisplay(observation).length > 30"> {{ getTvalDisplay(observation) }}<br /> </span>
                    <small>Click to preview file</small>
                  </q-tooltip>
                </div>
              </div>
              <!-- Questionnaire Display for Q type -->
              <div v-else-if="observation.VALTYPE_CD === 'Q'" class="questionnaire-display">
                <div class="questionnaire-value-display text-deep-purple clickable-value" @click="openQuestionnairePreview">
                  {{ getQuestionnaireDisplay(observation) }}
                  <q-tooltip class="bg-grey-9 text-white">
                    <span v-if="getQuestionnaireDisplay(observation).length > 30"> {{ getQuestionnaireDisplay(observation) }}<br /> </span>
                    <small>Click to preview questionnaire</small>
                  </q-tooltip>
                </div>
              </div>
              ...
              <div v-else class="text-grey-5 text-italic">No value recorded</div>
            </div>
        ...
        <div class="card-actions">
          ...
          <q-btn v-if="observation.VALTYPE_CD !== 'R'" flat round dense icon="edit" color="primary" size="sm" @click="startEdit">
    ...
    <!-- File Preview Dialog -->
    <FilePreviewDialog
      v-if="observation.VALTYPE_CD === 'R' && observation.TVAL_CHAR"
      v-model="showFileDialog"
      :observation-id="observation.OBSERVATION_ID"
      :file-info="JSON.parse(observation.TVAL_CHAR)"
      :concept-name="getConceptName(observation)"
      :upload-date="observation.START_DATE"
    />

    <!-- Questionnaire Preview Dialog -->
    <QuestionnairePreviewDialog
      v-if="observation.VALTYPE_CD === 'Q'"
      v-model="showQuestionnaireDialog"
      :observation-id="observation.OBSERVATION_ID"
      :concept-name="getConceptName(observation)"
      :completion-date="observation.START_DATE"
    />
  </q-card>
</template>
...
import QuestionnairePreviewDialog from 'components/shared/QuestionnairePreviewDialog.vue'
...
const valueInputType = computed(() => {
  ...
  switch (valtype) {
    ...
    case 'S':
      return 'selection'
    case 'Q':
      return 'questionnaire' // Questionnaire type
    case 'R':
      return 'file' // Raw data type should show file info
    case 'T':
    case 'A':
    default:
      return 'text'
...
const getQuestionnaireDisplay = (observation) => {
  // For questionnaire type (Q), get title from TVAL_CHAR and details from OBSERVATION_BLOB
  if (observation.VALTYPE_CD === 'Q') {
    const title = observation.TVAL_CHAR || 'Questionnaire'

    // Try to get response count from OBSERVATION_BLOB
    if (observation.OBSERVATION_BLOB) {
      try {
        const questionnaireData = JSON.parse(observation.OBSERVATION_BLOB)
        const responseCount = questionnaireData.responses?.length || 0
        return `${title} (${responseCount} responses)`
      } catch {
        // If parsing fails, just show title
      }
    }
  ...
  return observation.TVAL_CHAR || 'Questionnaire Data'
}

// getQuestionnaireData removed - dialog now fetches data itself using observation ID
...
.observation-card {
  ...
  &[data-value-type='Q'] {
    border-left: 4px solid #673ab7; // Deep purple for questionnaire
  }
}
