...
export function useLogger(customContext = null) {
  ...
  const logger = {
    ...
    // User action logging
    logAction: (action, details) => loggingStore.logUserAction(action, {
      component: context,
      ...details
    }),
    ...
    logFormValidation: (formName, isValid, errors) => loggingStore.debug(context, `Form validation: ${formName}`, {
      isValid,
      errors
    }),
    ...
    logDataLoad: (dataType, count, duration) => loggingStore.info(context, `Loaded ${dataType}`, {
      dataType,
      count,
      duration: `${duration}ms`
    }),
