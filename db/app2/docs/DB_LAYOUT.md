...
## 13. Konzept: DB-to-DB Datentransfer

### 13.1 Empfohlener Ansatz: ATTACH + Repository Hybrid

Der empfohlene Ansatz fuer den Einweg-Transfer (Migrate) von Patientendaten zwischen zwei SQLite-Datenbanken kombiniert SQLites nativen `ATTACH DATABASE`-Mechanismus mit dem bestehenden Repository Pattern.

**Kernidee:** Die Source-DB wird per `ATTACH` als zweite Datenbank in die bestehende Connection eingebunden. Daten werden direkt per SQL aus der Source gelesen und ueber die bestehenden Repositories in die Target-DB geschrieben. Nach Abschluss wird die Source per `DETACH` wieder getrennt.

### 13.2 Warum dieser Ansatz

| Ansatz | Vorteile | Nachteile |
|---|---|---|
| **ATTACH + Repository Hybrid** | Kein Dateikopieren, nutzt bestehende Repositories fuer Validierung und Audit-Felder, transaktionssicher, performant durch direkten SQL-Zugriff auf Source | Begrenzt auf SQLite-to-SQLite |
| Export/Import via HL7 | Format-unabhaengig | Langsam, Informationsverlust durch Mapping, grosser Overhead |
| Export/Import via JSON | Einfach zu implementieren | Kein Streaming, Memory-Probleme bei grossen Datenmengen |
| Raw File Copy | Maximal performant | Keine Merge-Moeglichkeit, ueberschreibt Target komplett |
| Zweite DB-Connection | Flexibel | Zwei separate Connections, keine Cross-DB-Queries moeglich |

**Entscheidung:** ATTACH + Repository Hybrid bietet die beste Balance aus Performance, Sicherheit und Wiederverwendung bestehender Bausteine.
...
│                     Transfer-UI                          │
│  ┌─────────────┐  ┌──────────────┐  ┌───────────────┐   │
│  │ Source-DB    │  │ Patienten-   │  │ Transfer-     │   │
│  │ auswaehlen   │  │ Auswahl      │  │ Fortschritt   │   │
│  └──────┬──────┘  └──────┬───────┘  └───────┬───────┘   │
│         │                │                   │           │
└─────────┼────────────────┼───────────────────┼───────────┘
          │                │                   │
    ┌─────▼────────────────▼───────────────────▼─────┐
    │              transfer-store (Pinia)             │
    │  state: sourceDbPath, selectedPatients,         │
    │         transferProgress, transferLog            │
    └────────────────────┬───────────────────────────┘
                         │
    ┌────────────────────▼───────────────────────────┐
    ...
    │  attachSource(path)                             │
    │  getSourcePatients()                            │
    │  getSourceVisits(patientNums)                   │
    │  getSourceObservations(encounterNums)           │
    │  getSourceConcepts(conceptCds)                  │
    │  transferPatients(selectedPatientNums)           │
    │  detachSource()                                 │
    └────────────────────┬───────────────────────────┘
                         │
    ┌────────────────────▼───────────────────────────┐
    ...
    │                                                 │
    │  DatabaseService ──→ Connection (ATTACH/DETACH) │
    │  PatientRepository ──→ createPatient()          │
    │  VisitRepository ──→ createVisit()              │
    │  ObservationRepository ──→ createObservation()  │
    │  ConceptRepository ──→ create()                 │
    └─────────────────────────────────────────────────┘
```

### 13.4 Transfer-Ablauf (Schritt fuer Schritt)

```
1. Benutzer waehlt Source-DB-Datei aus
   └── Datei-Dialog → sourceDbPath

2. ATTACH Source-DB
   └── SQL: ATTACH DATABASE 'source.db' AS source_db

3. Source-Daten lesen & anzeigen
   ├── SELECT * FROM source_db.PATIENT_DIMENSION
   └── Benutzer waehlt Patienten fuer Transfer

4. Transfer starten (in einer Transaktion)
   ├── Schritt A: Concepts transferieren
   ├── Schritt B: Patients transferieren
   ├── Schritt C: Visits transferieren
   └── Schritt D: Observations transferieren

5. DETACH Source-DB
   └── SQL: DETACH DATABASE source_db

6. Ergebnis anzeigen
   └── Zusammenfassung: X Patienten, Y Visits, Z Observations uebertragen
```

### 13.5 Datenfluss: Import-Reihenfolge & Referentielle Integritaet

Die Reihenfolge ist entscheidend, da Foreign Keys eingehalten werden muessen:

```
Schritt A: CONCEPT_DIMENSION (keine Abhaengigkeiten)
   │
   │  Concepts muessen zuerst existieren, da OBSERVATION_FACT
   │  auf CONCEPT_CD verweist.
   │
Schritt B: PATIENT_DIMENSION (keine DB-Abhaengigkeiten)
   │
   │  Patienten muessen vor Visits existieren,
   │  da VISIT_DIMENSION.PATIENT_NUM → PATIENT_DIMENSION.
   │
   │  ⚠ PATIENT_NUM aendert sich in der Target-DB!
   │  → Mapping: source_PATIENT_NUM → target_PATIENT_NUM
   │
Schritt C: VISIT_DIMENSION (abhaengig von PATIENT_DIMENSION)
   │
   │  Visits muessen vor Observations existieren,
   │  da OBSERVATION_FACT.ENCOUNTER_NUM → VISIT_DIMENSION.
   │
   │  ⚠ ENCOUNTER_NUM aendert sich in der Target-DB!
   │  → Mapping: source_ENCOUNTER_NUM → target_ENCOUNTER_NUM
   │
Schritt D: OBSERVATION_FACT (abhaengig von PATIENT + VISIT + CONCEPT)
   │
   │  Verwendet die gemappten PATIENT_NUM und ENCOUNTER_NUM
   │  aus den vorherigen Schritten.
   │
   └── ✓ Transfer abgeschlossen
```

**ID-Mapping:** Da AUTOINCREMENT-IDs in der Target-DB neu vergeben werden, muss ein Mapping gefuehrt werden:

```javascript
// Mapping-Objekte waehrend des Transfers
const patientNumMap = {}   // { sourcePatientNum: targetPatientNum }
const encounterNumMap = {} // { sourceEncounterNum: targetEncounterNum }
```
...
| Datei | Zweck |
|---|---|
| `src/core/services/database-transfer-service.js` | Kernlogik: ATTACH, Lesen, Mapping, Schreiben, DETACH |
| `src/stores/transfer-store.js` | Pinia Store: State (sourceDbPath, selectedPatients, progress, log) |
| `src/views/TransferView.vue` (oder Modal) | UI: Source auswaehlen, Patienten waehlen, Transfer starten, Fortschritt |

### 13.7 Bestehende Bausteine die wiederverwendet werden

| Baustein | Verwendung im Transfer |
|---|---|
| `DatabaseService.executeQuery()` | SQL-Abfragen auf die attached Source-DB |
| `DatabaseService.executeTransaction()` | Atomarer Transfer aller Daten |
| `PatientRepository.createPatient()` | Patienten in Target anlegen (inkl. Validierung, Audit-Felder) |
| `VisitRepository.createVisit()` | Visits in Target anlegen |
| `ObservationRepository.createObservation()` | Observations in Target anlegen |
| `ConceptRepository.create()` | Fehlende Concepts in Target anlegen |
| `PatientRepository.findByPatientCode()` | Duplikat-Erkennung ueber PATIENT_CD |
| `ElectronConnection` | Bestehende DB-Connection fuer ATTACH/DETACH |
...
```sql
-- Source-DB einbinden
ATTACH DATABASE '/pfad/zur/source.db' AS source_db;

-- Pruefen ob ATTACH erfolgreich war
SELECT name FROM pragma_database_list WHERE name = 'source_db';
...
```sql
-- Alle Patienten aus Source lesen
SELECT * FROM source_db.PATIENT_DIMENSION;

-- Visits fuer ausgewaehlte Patienten
SELECT * FROM source_db.VISIT_DIMENSION
WHERE PATIENT_NUM IN (1, 2, 3);

-- Observations fuer diese Visits
SELECT * FROM source_db.OBSERVATION_FACT
WHERE ENCOUNTER_NUM IN (10, 11, 12, 13);

-- Benoetigte Concepts ermitteln (nur die, die in Target fehlen)
SELECT DISTINCT s.CONCEPT_CD, s.NAME_CHAR, s.VALTYPE_CD, s.UNIT_CD,
       s.CONCEPT_PATH, s.CATEGORY_CHAR, s.CONCEPT_BLOB, s.SOURCESYSTEM_CD
FROM source_db.OBSERVATION_FACT o
JOIN source_db.CONCEPT_DIMENSION s ON o.CONCEPT_CD = s.CONCEPT_CD
WHERE o.PATIENT_NUM IN (1, 2, 3)
  AND s.CONCEPT_CD NOT IN (SELECT CONCEPT_CD FROM main.CONCEPT_DIMENSION);
```

### 13.9 Duplikat-Handling

Duplikate werden ueber den natuerlichen Schluessel `PATIENT_CD` erkannt (nicht ueber die Auto-ID `PATIENT_NUM`):

```
Fuer jeden Source-Patienten:
  1. findByPatientCode(source.PATIENT_CD)
  2. Falls gefunden → Patient ueberspringen (SKIP)
     └── Log: "Patient [PATIENT_CD] existiert bereits, uebersprungen"
  3. Falls nicht gefunden → createPatient()
     └── Neuer PATIENT_NUM wird im Mapping gespeichert

Fuer Concepts:
  1. CONCEPT_CD ist bereits UNIQUE PK in CONCEPT_DIMENSION
  2. Nur Concepts importieren die in Target nicht existieren (siehe SQL oben)
  3. INSERT OR IGNORE fuer maximale Sicherheit
```

**Strategie: Skip-on-Duplicate.** Existierende Daten in der Target-DB werden nie ueberschrieben. Nur neue Patienten (und deren komplette Kette) werden hinzugefuegt. Das verhindert versehentlichen Datenverlust und haelt den Transfer idempotent.
