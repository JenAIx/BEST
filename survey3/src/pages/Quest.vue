...
export default {
  ...
  methods: {
    ...
    // PDF EXPORT — einfaches, druckbares Leerformular (siehe src/tools/quest-pdf.js)
    exportPdf() {
      ...
      // Bilder liegen unter <appBase>/img/; im Druckfenster braucht es absolute URLs.
      const imgBase = `${window.location.origin}${import.meta.env.BASE_URL}img/`

      const win = window.open('', '_blank')
      if (!win) return

      win.document.write(buildQuestPdfHtml(quest, { dateStr, timeStr, imgBase }))
      win.document.close()
      win.focus()
      win.print()
    },
